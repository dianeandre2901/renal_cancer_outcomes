"""
 Patch-Level Binary Classifier with tissue coordinates pre computed - images + tabular passed to model 
100 slides for training - 100 for validation
...
class MultiModalPatchDataset(Dataset):
    def __init__(self, patch_df, transform=None, max_patches_per_slide=None):
        if max_patches_per_slide:
            patch_df = patch_df.groupby("slide_id").apply(
...
train_dataset = MultiModalPatchDataset(first_100_train, transform=transform, max_patches_per_slide=patch_cap)
...
for epoch in range(epochs):
    ...
    plt.tight_layout()
    plt.savefig(f"results/plots/CNN_binary_images100+tab_slide_confusion_matrix_epoch{epoch + 1}.png")
    plt.close(fig)
    print(f"Saved confusion matrix as results/plots/CNN_binary_images+tab100_slide_confusion_matrix_epoch{epoch + 1}.png/pdf")
    ...
    plt.tight_layout()
    plt.savefig(f"results/plots/CNN_binary_images+tab100_slide_ROC_epoch{epoch + 1}.png")
    plt.close()
    print(f"Saved ROC curve as results/plots/CNN_binary_images+tab100_slide_ROC_epoch{epoch + 1}.png")
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), "model_CNN_binary_images+tab100.pth")
    else:
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/CNN_binary_images+tab100_train_val_loss_acc.png")
plt.close()
print("Saved train/val loss & acc plot to results/plots/CNN_binary_images100_train_val_loss_acc.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/CNN_binary_images50_train_val_loss_separate.png")
plt.close()
print("Saved loss calibration plot to results/plots/CNN_binary_images+tab100_train_val_loss_separate.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/CNN_binary_images50_train_val_acc_separate.png")
plt.close()
print("Saved accuracy calibration plot to results/plots/CNN_binary_images=+ab100_train_val_acc_separate.png")
...
# Save to file
with open("results/plots/CNN_binary_images+tab100_runtime_aftertuning.txt", "w") as f:
