...
first_20_val   = val_patches[val_patches['slide_id'].isin(val_patches['slide_id'].unique()[:100])]
...
val_dataset   = PrecomputedPatchDataset(first_20_val, df_val, transform=transform, max_patches_per_slide=patch_cap)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/model4_train_val_loss_acc_deepsurv_20.png")
plt.savefig("results/plots/model4_train_val_loss_acc_deepsurv_20.pdf")
plt.close()
print("Saved train/val loss & acc plot to results/plots/model4_train_val_loss_acc_deepsurv_20.png/pdf")
...
# Save to file
with open("results/plots/model4_deepsurv_20_runtime.txt", "w") as f:
