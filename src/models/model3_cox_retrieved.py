...
tqdm.pandas()  
tqdm_settings = dict(disable=True)
# 
...
for epoch in range(epochs):
    ...
    if np.std(slide_mean_risk) == 0 or np.isnan(slide_mean_risk).any():
       print("Warning: Risk scores are not varying -> skipping normalization.")
       slide_mean_risk = slide_mean_risk  # no-op
    else:
       slide_mean_risk = (slide_mean_risk - np.mean(slide_mean_risk)) / np.std(slide_mean_risk)
    
    mask = ~np.isnan(slide_mean_risk) & ~np.isnan(slide_times) & ~np.isnan(slide_events)
    cindex = concordance_index(-slide_mean_risk[mask], slide_times[mask], slide_events[mask])
...
threshold = np.median(slide_mean_risk)
slide_pred = (slide_mean_risk > threshold).astype(int) # convert to binary: risk > 0 = high-risk
...
np.save("results/slide_risks_deepsurv.npy", slide_mean_risk)
np.save("results/slide_events_deepsurv.npy", slide_events)
np.save("results/slide_times_deepsurv.npy", slide_times)
