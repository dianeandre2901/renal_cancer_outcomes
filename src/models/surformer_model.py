...
class SurformerMIL(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.output(x[:, 0])  # return risk score
...
def train_epoch(model, dataloader, optimizer):
    ...
    for x, time, event in dataloader:
        if torch.isnan(x).any() or torch.isnan(time).any() or torch.isnan(event).any():
...
def eval_epoch_loss(model, dataloader):
    ...
    with torch.no_grad():
        for x, time, event in dataloader:
            if x.numel() == 0 or torch.isnan(x).any() or torch.isnan(time).any() or torch.isnan(event).any():
                continue
            x, time, event = x.to(device), time.to(device), event.to(device)
            risk = model(x)
            loss = cox_ph_loss(risk, time, event)
...
if __name__ == "__main__":
    ...
    x_sample, time_sample, event_sample = next(iter(train_loader))
    print("Sample input shape:", x_sample.shape)
    print("Sample time/event:", time_sample, event_sample)
