"""
CNN Cox Model with images and tabular features. 100 training slides - 100 validation slides
This model trains an EfficientNet-B0 to estimate patch-level survival risk scores
from precomputed tissue coordinates and tabular clinical features. Patches inherit slide-level event labels.
...
df_val = df_val[['slide_id', 'os_days', 'event','tumour_grade', 'tumour_stage', 'age_at_diagnosis_years']]
...
class PrecomputedPatchDataset(Dataset):
    def __init__(self, patch_csv, clinical_df, transform=None, max_patches_per_slide=None):
        ...
        # Merge clinical data to get os_days and event per patch plus tabular features
    ...
    def __getitem__(self, idx):
        ...
        tabular_features = torch.tensor([
            row["tumour_grade"],
            row["tumour_stage"],
            row["age_at_diagnosis_years"]
        ], dtype=torch.float32)
        if self.transform:
            patch = self.transform(patch)
        return patch, tabular_features, slide_id, time, event
...
def skip_censored_collate_fn(batch):
    imgs, tabulars, slide_ids, times, events = zip(*batch)
...
# Take the first 100 unique slides (and all their patches)
first_100_train = train_patches[train_patches['slide_id'].isin(train_patches['slide_id'].unique()[:100])]
first_100_val   = val_patches[val_patches['slide_id'].isin(val_patches['slide_id'].unique()[:100])]
patch_cap = None # or None for all

train_dataset = PrecomputedPatchDataset(first_100_train, df_train, transform=transform, max_patches_per_slide=patch_cap)
...
class MyEffNet(nn.Module):
    def __init__(self, base_model):
        ...
        self.tabular_fc = nn.Linear(3, 32)
        self.classifier = nn.Linear(base_model.classifier[1].in_features + 32, 1)
    def forward(self, x_img, x_tab):
        x_img = self.features(x_img)
        x_img = self.avgpool(x_img)
        x_img = torch.flatten(x_img, 1)
        x_img = self.dropout(x_img)
        x_tab = torch.relu(self.tabular_fc(x_tab))
...
for epoch in range(epochs):
    ...
    for batch in train_loader:
        ...
        imgs, tabulars, slide_ids, times, events = batch
        imgs, tabulars, times, events = imgs.to(device), tabulars.to(device), times.to(device), events.to(device)
        optimizer.zero_grad()
        risk_scores = model(imgs, tabulars)
    ...
    if cindex > best_val_cindex:
        ...
        torch.save(model.state_dict(), "cox_images100.pth")
    else:
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/train_val_loss_acc_cox_images100.pdf")
plt.close()
print("Saved train/val loss & acc plot to results/plots/train_val_loss_cox_images100.png")
...
if best_model_state is not None:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/plots/cox_images100_cstat_curve.png")
    plt.close()
    print("Saved C-statistic performance curve to results/plots/cox_images100_cstat_curve.png")
...
# Save to file
with open("results/plots/cox_images100_runtime.txt", "w") as f:
