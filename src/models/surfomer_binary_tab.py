...
class PrecomputedFeatureBinaryDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        row = self.clinical_df.loc[slide_id]
        label = torch.tensor(row["vital_status"]).float()
        tabular = torch.tensor(row[["age_at_diagnosis_years", "tumour_grade", "tumour_stage"]].astype(float).values, dtype=torch.float32)
        return features, tabular, label


# === SURFORMER BACKBONE ===

class SurformerMIL(nn.Module):
    def __init__(self, dim=512, tabular_dim=3, heads=8, layers=2):
        super().__init__()
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        self.tabular_fc = nn.Linear(tabular_dim, dim)
    ...
    def forward(self, x, tabular):
        ...
        tab_embed = self.tabular_fc(tabular).unsqueeze(1)  # (B, 1, dim)
        cls_tokens = self.cls_token.expand(B, -1, -1)
        x = torch.cat((cls_tokens, tab_embed, x), dim=1)
...
class WSI_BinaryModel(nn.Module):
    ...
    def forward(self, x, tabular):
        ...
        return self.surformer(x, tabular).squeeze(-1)
...
def train_epoch(model, dataloader, optimizer, criterion):
    ...
    for x, tabular, label in dataloader:
        x, tabular, label = x.to(device), tabular.to(device), label.to(device)
        logits = model(x, tabular)
...
# === Post-training Plots ===
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("results/plots/surformer_binary_loss_img+tab_curve.png")
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig("results/plots/surformer_binary_img+tab_accuracy_curve.png")
    plt.close()
    ...
    all_probs = []
    with torch.no_grad():
        for x, tabular, label in val_loader:
            x = x.to(device)
    ...
    plt.grid(True)
    plt.savefig("results/plots/surformer_binary_img+tab_roc_auc.png")
    plt.close()
...
# Save to file
os.makedirs("results/plots", exist_ok=True)
with open("results/plots/surformer_binary_img+taB_runtime.txt", "w") as f:
