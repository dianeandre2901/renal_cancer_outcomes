...
train_dataset = PrecomputedPatchDataset(first_60_train, df_train, transform=transform, max_patches_per_slide=patch_cap)
val_dataset   = PrecomputedPatchDataset(first_60_val, df_val, transform=transform, max_patches_per_slide=patch_cap)
...
for epoch in range(epochs):
    ...
    if cindex > best_val_cindex:
        ...
        torch.save(model.state_dict(), "best_model_cox2_60slides.pth")
    else:
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Model 4: Train/Val Loss per Epoch")
plt.legend(loc="center right")
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/model4_train_val_loss_acc_cox2_60.png")
plt.savefig("results/plots/model4_train_val_loss_acc_cox2_60.pdf")
plt.close()
print("Saved train/val loss & acc plot to results/plots/model4_train_val_loss_acc_cox2_60.png/pdf")
...
# Save to file
with open("results/plots/model4_cox_60slides_runtime.txt", "w") as f:
