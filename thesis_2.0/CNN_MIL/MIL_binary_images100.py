...
train_dataset = PrecomputedPatchDataset(first_100_train, transform=transform, max_patches_per_slide=patch_cap)
...
for epoch in range(epochs):
    ...
    plt.tight_layout()
    plt.savefig(f"results/plots/MIL_binary_images100_slide_confusion_matrix_epoch{epoch + 1}.png")
    plt.close(fig)
    print(f"Saved confusion matrix as results/plots/MIL_binary_images100_slide_confusion_matrix_epoch{epoch + 1}.png/pdf")
    ...
    if val_acc > best_val_acc:
        ...
        torch.save(model.state_dict(), "model_MIL_binary_images100.pth")
    else:
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/MIL_binary_images100_train_val_loss_acc.png")
plt.close()
print("Saved train/val loss & acc plot to results/plots/MIL_binary_images100_train_val_loss_acc.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/MIL_binary_images100_train_val_loss_separate.png")
plt.close()
print("Saved loss calibration plot to results/plots/MIL_binary_images100_train_val_loss_separate.png")
...
plt.grid(True)
plt.tight_layout()
plt.savefig("results/plots/MIL_binary_images100_train_val_acc_separate.png")
plt.close()
print("Saved accuracy calibration plot to results/plots/MIL_binary_images100_train_val_acc_separate.png")
...
if len(np.unique(slide_trues_filtered)) == 2:
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("results/plots/MIL_binary_images100_final_slide_ROC.png")
    plt.close()
    print("Saved final ROC curve to results/plots/MIL_binary_images100_final_slide_ROC.png")
else:
...
# Save to file
with open("results/plots/MIL_binary_images100_runtime_aftertuning.txt", "w") as f:
